...
def generate_technical_questions(state: InterviewState) -> InterviewState:
    ...
    question_list=state.get("list_of_questions", {})
    previous_responses = get_prev(question_list, "technical")
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        create ONE technical question that will assess the candidate's expertise.
        
        List of instructions for the question to be asked:
        1. Make it specific to the required skills
        2. The question should be 1 line long. where the expected answer should be 2-3 lines.
        3. try to ask questions based on condidates knowlege on that topic.
        4. dont ask to write code or deign some thing. it just should be 2-3 liner answers
        5 .if the candidate has mentions a skill multiple times in resume ansd it is also in job description try to ask question on that topic.
        6. if the candidate answers well, try to increase the difficulty of the question.
        7. Include expected answer that would represent a good response
        8.questions in that area. If they showed weakness, ask about a different area.
    ...
    try:
        ...
        try:
            if isinstance(response, str):
                ...
                modified_text = response[response.find('{'):response.rfind('}')+1]

                question = json.loads(modified_text)
            else:
        ...
        question_list=state["list_of_questions"] 
        ...
        question_list=state["list_of_questions"] 
        question_list.append(default_question)
        return {
            **state, 
            "list_of_questions": question_list,
            "technical_questions": technical_questions
...
def generate_project_questions(state: InterviewState) -> InterviewState:
    ...
    try:
        ...
        question_list=state.get("list_of_questions", {})
        previous_responses = get_prev(question_list, "project")
        prompt = ChatPromptTemplate.from_messages([
        ("system", """You are an expert technical interviewer.
        Analyze the key projects and work experiences listed in the provided resume. Generate one highly specific, non-generic interview questions that require concise, two-line answers:
        ...
        2. VERY IMPORTANT: question should be bit simple and the question should be 2 lines maximum
        3. dont club 2 questions in one.ask only one question at a time. 
        4.If the candidate showed strength in a specific area, you can ask more challenging questions in that area. If they showed weakness, ask about a different area.
        ...
        Return ONLY a valid JSON object with exactly these fields:
        {{
            "question": "The complete interview question text where question  is 1-2 lines long" ,
            "expected_answer": "2 line answer to the question"
        }}
       ...
       """),
        ...
        "question_number": current_question_index + 1,
        "project_responses": previous_responses
        }).content
        ...
        **state, 
        ...
        "list_of_questions": question_list
            ...
            if len(project_questions) < len(default_questions):
                q=default_questions[len(project_questions)]
            ...
            project_questions.append(q)
            return {**state, "project_questions": project_questions,"list_of_questions": question_list}
...
"""
        ...
        **state, 
        ...
        "behavioral_questions":behavioral_questions
            ...
            question_list=state["list_of_questions"]
            question_list.append(default_q)
            
            return {**state, "behavioral_questions": behavioral_questions,"list_of_questions": question_list}
...
Create 1 behavioral interview question that assesses ability to handle real-world challenges in the workplace .
...
"""
        ...
        question_list=state["list_of_questions"]
        question_list.append(default_q)
        return {**state, "scenario_questions": scenario_questions,"list_of_questions": question_list}

def auto_repaid(response_text):
    fix_prompt=f"""The following text is supposed to be a JSON object, but it may have formatting errors.
            Please fix any JSON syntax errors and return ONLY the corrected valid JSON.
            in case of any {{  }} are missing plz add.
            Original text:
            {response_text}
...
def perform_assessment(state: InterviewState) -> Dict:
    ...
    job_data = state["job_data"]
    match_analysis = state["match_analysis"]
    resume_fit=match_analysis.get("skill_match_percentage", "30")

    responses = state["list_of_questions"]
    ...
    try:
        ...
        try:
            if isinstance(assessment, str):
                # Try to parse the string as JSON if it's a string
                modified_text = assessment[assessment.find('{'):assessment.rfind('}')+1]
                # Try to parse the string as JSON if it's a string
                assessment = json.loads(modified_text)
        except Exception as e:
            assessment=auto_repaid(assessment)

        # print(f"the respones for the interview are {responses}")
        print(f"Assessment generated: {assessment}")
        ...
        assessment["resume_fit"]=int(resume_fit)
        ...
        default_assessment = {
            ...
            "resume_fit": int(resume_fit) or 50,
...
def evaluate_response(question_text, response: str) -> dict:
...
def get_prev(question_list, catigory):
    """Get the previous question of a specific category."""
    resp=[]
    for question in question_list:
        if question.get("question_type") == catigory:
            resp.append(question)
    print("the previous question is", resp)
    return resp
